...
const ProductContainer = () => {
    ...
    console.log("Products:", products); // 전체 상품 출력
    console.log("Target ID:", id); // 사용자가 찾으려는 ID 확인

    const foundProduct = products?.find((pr) => String(pr.id) === String(id));

    const productList = products.map(({id, productName, productFilePath, productFileName, productPrice, productDiscount, productReviewStar}, index) => (
        <S.Product key={index}>
            <S.ProductImageWrap>
                <HeartBtn/>
                <Link to={`read/${index}`}>
                    <img src={`${process.env.PUBLIC_URL}/assets/images/products/${productFileName}`} alt={"상품" + (index + 1)} />
                </Link>
            </S.ProductImageWrap>
            <S.ProductInfo>
                <Link to={`read/${index}`}>
